...
SystemOpts(max_particles;
    ...
    origin = (0, 0, 0)
) = SystemOpts(Float32(emit_rate), UInt(max_emit), UInt(max_particles), isnothing(reset_on_done) ? nothing : UInt(reset_on_done), SVector{3, Float32}(origin))
...
struct Particles
    pos::Vector{NTuple{3, Cfloat}}
    velocity::Vector{NTuple{3, Cfloat}}
    acceleration::Vector{NTuple{3, Cfloat}}
...
function set_alive!(p::Particles, val::UInt)
    @ccall ffi__set_alive(p.__ptr::Ptr{CParticles}, val::Csize_t)::Cvoid
end
...
function particles(c_particles_ptr::Ptr{CParticles})
    ...
    return Particles(pos_vecs, vel_vecs, acc_vecs, color_vecs, alive_vec, lifetime_vec, size_vec, max_sz, () -> UInt(unsafe_load(c_particles_ptr).alive_count), c_particles_ptr)
end
...
function __generator(thunk_id::Ptr{Cvoid}, c_particles_ptr::Ptr{CParticles}, dt::Cfloat, start_ix::Csize_t, end_ix::Csize_t)::Cvoid
    ...
    thunk.generator(particles(c_particles_ptr), dt, start_ix + 1, end_ix)
end
...
function color_lerp(start_color::NTuple{4, UInt8}, end_color::NTuple{4, UInt8}, factor::Float32)::NTuple{4, UInt8}
...
circle_gen! = OnCircleGen((0f0, 0f0, 0f0), 5f0, 10f0)

function generator(p::Particles, dt::Float32, start_ix::UInt, end_ix::UInt)
...
function updater!(p::Particles, dt::Float32)
    elapsed_time = @elapsed begin
...
function __main()
    c_particles_ptr = @ccall ffi__get_particles()::Ptr{CParticles}
    p = particles(c_particles_ptr)

    n = 10
    elapsed_time = @elapsed begin
        for i in 1:n
            # generator(p, 0.005f0, UInt(1), p.max_size)
            set_alive!(p, p.max_size)
            updater!(p, 0.005f0)
        end
    end
    println(elapsed_time/n)

    # new_system(generator, updater, SystemOpts(300000, origin=(0, 10, 0)))
end
