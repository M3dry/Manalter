...
// I don't think there's anything that can be done about this, as the Enemy class requires the model information to work
// properly
...
TEST_CASE("Spawn every second", "[enemy][raylib]") {
    ...
    for (int i = 0; i < TICKS * n; i++) {
        enemies.tick(shapes::Circle(Vector2Zero(), 1.0f), enemy_models, Vector2Zero());
    }
