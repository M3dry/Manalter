...
Player::Player(Vector3 position)
    ...
    hitbox = shapes::Polygon((Vector2){ max.x + min.x, max.z + min.z }, { (Vector2){ min.x, max.z }, (Vector2){ min.x, min.z }, (Vector2){ max.x, min.z }, (Vector2){ max.x, max.z } });
}
...
void Player::update_in_reach(Vector2 mouse) {
...
PlayerStats::PlayerStats(uint32_t max_health, uint32_t max_mana, uint8_t max_spells)
    : max_health(max_health), max_mana(max_mana), health(max_health),
      mana(max_mana), lvl(0), exp(0), exp_to_next_lvl(100),
      max_spells(max_spells), equipped_spells(10, UINT32_MAX) {};
...
void PlayerStats::tick_cooldown() {
...
void PlayerStats::cast_equipped(int idx) {
    if (idx >= 10 || idx >= max_spells || equipped_spells[idx] == UINT32_MAX) return;
...
void ItemDrop::draw_name(std::function<Vector2(Vector3)> to_screen_coords) const {
    ...
    auto name = get_name().c_str();
    DrawText(name, pos.x, pos.y, 20, WHITE);
}

void ItemDrop::move_item(std::function<void(Spell&&)> spell_f) {
    switch (type) {
        case Type::Spell: return spell_f(std::move(std::get<Spell>(item)));
    }
}

const std::string& ItemDrop::get_name() const {
    switch (type) {
        case Type::Spell:
...
Loop::Loop(int width, int height)
    : screen((Vector2){ (float)width, (float)height }), player((Vector3){ 0.0f, 10.0f, 0.0f }),
      player_stats(100, 100, 4), mouse_pos(Vector2Zero()), assets(screen),
      registered_keys({ KEY_N, KEY_M, KEY_ONE, KEY_TWO, KEY_THREE, KEY_FOUR, KEY_FIVE, KEY_SIX, KEY_SEVEN, KEY_EIGHT, KEY_NINE, KEY_ZERO }) {
#ifdef PLATFORM_WEB
    ...
    item_drops.emplace_back((Vector2){ 200.0f, 200.0f },
                            Spell(Spell::Fire_Wall, Rarity::Epic, 30));
    item_drops.emplace_back((Vector2){ 200.0f, 150.0f },
                            Spell(Spell::Frost_Nova, Rarity::Epic, 30));
    item_drops.emplace_back((Vector2){ 200.0f, 100.0f },
                            Spell(Spell::Falling_Icicle, Rarity::Epic, 30));
};
    ...
    BeginTextureMode(assets[assets::Target, false]);
        ...
        BeginMode3D(player.camera);
            ...
            for (const auto& item_drop : item_drops) {
                DrawCircle3D((Vector3){ item_drop.hitbox.center.x, 1.0f, item_drop.hitbox.center.y }, item_drop.hitbox.radius, (Vector3){ 1.0f, 0.0f, 0.0f }, 90.0f, RED);
            }
