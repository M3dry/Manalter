...
namespace engine::model {
    ...
    std::expected<ModelId, std::string> load(GPUGroupId group_id, const char* file_path, bool binary, std::string* warning = nullptr);
    std::expected<ModelId, std::string> load(GPUGroupId group_id, std::span<std::byte> data, bool binary, std::string* warning = nullptr);
