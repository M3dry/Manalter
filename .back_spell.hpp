...
using Spell = struct Spell {
    ...
    static const std::array<uint32_t, NameSize> damage_map;
    static const std::array<std::pair<uint32_t, uint32_t>, NameSize> damage_randomization_map;
    // changes damage
    ...
    int get_base_manacost() const;
    const std::string& get_icon_path() const;
    uint32_t get_base_damage() const;
    const std::pair<uint32_t, uint32_t>& get_damage_randomization() const;
